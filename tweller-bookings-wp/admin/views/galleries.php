...
<div class="wrap tf2-wrap">
    ...
    <!-- Manual Upload Panel -->
    <div class="tf2-card tf2-mb-6" id="tf2-manual-upload-panel">
        ...
        <div style="display:flex; gap:12px; align-items:flex-end; flex-wrap:wrap; margin-bottom:16px;">
            <div>
                ...
                <select id="tf2-upload-session" style="padding:7px 10px; border:1px solid #D1D5DB; border-radius:6px; font-size:13px; min-width:260px; font-family:inherit;">
        ...
        <!-- Drop zone -->
        <div id="tf2-drop-zone" style="border:2px dashed #D1D5DB; border-radius:10px; padding:40px 20px; text-align:center; cursor:pointer; transition:border-color .2s, background .2s; background:#FAFAFA;">
            ...
            <input type="file" id="tf2-file-input" accept="image/jpeg,image/png,image/webp" multiple style="display:none;">
            <button type="button" id="tf2-browse-btn" class="tf2-btn tf2-btn--secondary tf2-btn--sm">Browse files</button>
        </div>

        <!-- File list -->
        <div id="tf2-file-list" style="margin-top:12px; display:none;">
            <div style="font-size:12px; color:#6B7280; margin-bottom:8px;"><span id="tf2-file-count">0</span> file(s) selected</div>
            <div id="tf2-file-names" style="max-height:120px; overflow-y:auto; font-size:12px; color:#374151; background:#F9FAFB; border:1px solid #E5E7EB; border-radius:6px; padding:8px 12px;"></div>
        </div>

        <!-- Progress -->
        <div id="tf2-upload-progress" style="display:none; margin-top:12px;">
            <div style="height:6px; background:#E5E7EB; border-radius:3px; overflow:hidden;">
                <div id="tf2-progress-bar" style="height:100%; background:#6366F1; width:0%; transition:width .3s;"></div>
            </div>
            <p id="tf2-progress-text" style="font-size:12px; color:#6B7280; margin:6px 0 0;"></p>
        </div>

        <!-- Result -->
        <div id="tf2-upload-result" style="display:none; margin-top:12px;"></div>

        <div style="margin-top:16px; display:flex; gap:8px;">
            <button type="button" id="tf2-upload-btn" class="tf2-btn tf2-btn--primary" disabled>Upload Photos</button>
            <button type="button" id="tf2-clear-btn" class="tf2-btn tf2-btn--ghost" style="display:none;">Clear</button>
        </div>
    </div>
    ...
    (function($){
        ...
        function resetUI() {
            ...
            $('#tf2-file-input').val('');
            $('#tf2-file-list').hide();
            $('#tf2-file-names').empty();
            $('#tf2-file-count').text('0');
            $('#tf2-upload-btn').prop('disabled', true);
            $('#tf2-clear-btn').hide();
            $('#tf2-upload-result').hide().empty();
            $('#tf2-upload-progress').hide();
            $('#tf2-progress-bar').css('width','0%');
            $('#tf2-drop-zone').css({borderColor:'#D1D5DB', background:'#FAFAFA'});
        }

        function showFiles(newFiles) {
            files = Array.from(newFiles);
            if (!files.length) return;
            $('#tf2-file-count').text(files.length);
            $('#tf2-file-names').html(files.map(function(f){ return '<div>'+$('<span>').text(f.name).html()+'</div>'; }).join(''));
            $('#tf2-file-list').show();
            $('#tf2-upload-btn').prop('disabled', !$('#tf2-upload-session').val());
            $('#tf2-clear-btn').show();
        }

        $('#tf2-browse-btn').on('click', function(){ $('#tf2-file-input').trigger('click'); });
        $('#tf2-file-input').on('change', function(){ showFiles(this.files); });
        $('#tf2-clear-btn').on('click', resetUI);
        $('#tf2-upload-session').on('change', function(){
            $('#tf2-upload-btn').prop('disabled', !$(this).val() || !files.length);
        });

        var dz = document.getElementById('tf2-drop-zone');
        ...
        $('#tf2-upload-btn').on('click', function(){
            var sessionCode = $('#tf2-upload-session').val();
            if (!sessionCode || !files.length) return;

            var total = files.length, done = 0, savedNames = [], errorMsgs = [];
            $('#tf2-upload-progress').show();
            $('#tf2-upload-result').hide().empty();
            $('#tf2-upload-btn').prop('disabled', true);
            ...
            function uploadNext(idx) {
                if (idx >= total) {
                    // All done
                    $('#tf2-progress-bar').css('width','100%');
                    var html = '';
                    if (savedNames.length) {
                        html += '<div class="tf2-alert tf2-alert--success">Uploaded ' + savedNames.length + ' photo(s) successfully.</div>';
                    }
                    if (errorMsgs.length) {
                        html += '<div class="tf2-alert tf2-alert--error" style="margin-top:8px;">' + errorMsgs.join('<br>') + '</div>';
                    }
                    $('#tf2-upload-result').html(html).show();
                    setTimeout(function(){ location.reload(); }, 1500);
                    return;
                }

                var pct = Math.round((idx / total) * 100);
                $('#tf2-progress-bar').css('width', pct + '%');
                $('#tf2-progress-text').text('Uploading ' + (idx+1) + ' of ' + total + ': ' + files[idx].name);
                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:16px; flex-wrap:wrap; gap:12px;">
                    ...
                    <div style="display:flex; gap:8px; align-items:center;">
                        <?php if ( $client_link && $gallery_info['photo_count'] > 0 ) : ?>
                            <a href="<?php echo esc_url( $client_link ); ?>" target="_blank" class="tf2-btn tf2-btn--secondary tf2-btn--sm" style="font-size:12px;">Client View</a>
                        <?php endif; ?>
                        <a href="<?php echo admin_url( 'admin.php?page=tweller-flow-2-session&id=' . $session->id . '#tf2-gallery-manager' ); ?>" class="tf2-btn tf2-btn--secondary tf2-btn--sm" style="font-size:12px;">&#9998; Edit Gallery</a>
                        <a href="<?php echo admin_url( 'admin.php?page=tweller-flow-2-session&id=' . $session->id ); ?>" class="tf2-btn tf2-btn--ghost tf2-btn--sm" style="font-size:12px;">Session Details</a>
                    </div>
                </div>
                ...
                <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; padding-top:16px; border-top:1px solid #F3F4F6;">
                    ...
                    <div style="display:flex; align-items:center; gap:8px;">
                        ...
                        <form method="post" style="display:inline-flex; gap:4px; align-items:center;">
                            ...
                            <button type="submit" class="tf2-btn tf2-btn--secondary tf2-btn--sm" style="font-size:11px; padding:5px 10px;">Set</button>
                        </form>
                        <?php if ( $gallery_info['has_password'] ) : ?>
                            <form method="post" style="display:inline;">
                                ...
                                <input type="hidden" name="redirect_to" value="galleries">
                                <button type="submit" class="tf2-btn tf2-btn--ghost tf2-btn--sm" style="font-size:11px; padding:5px 10px; color:#DC2626;" onclick="return confirm('Remove password protection?');">Remove</button>
                            </form>
                        <?php endif; ?>
                    </div>
