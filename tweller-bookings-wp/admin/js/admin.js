...
(function() {
    ...
    function initTabs() {
        ...
        var tabs = tabContainer.querySelectorAll('.tf2-tab');
        tabs.forEach(function(tab) {
            tab.addEventListener('click', function() {
                var targetId = 'tab-' + this.getAttribute('data-tab');

                // Deactivate all tabs and content
                tabs.forEach(function(t) { t.classList.remove('tf2-tab--active'); });
                document.querySelectorAll('.tf2-tab-content').forEach(function(c) {
                    c.classList.remove('tf2-tab-content--active');
                });

                // Activate clicked tab and content
                this.classList.add('tf2-tab--active');
                var target = document.getElementById(targetId);
                if (target) target.classList.add('tf2-tab-content--active');
            });
        });
    }
